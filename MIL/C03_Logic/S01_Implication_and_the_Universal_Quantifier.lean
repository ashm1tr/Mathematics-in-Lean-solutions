...
#check my_lemma2 h₀ h₁ ha hb -- don't have to mention a, b, δ
-- only applies when USING the lemma
end

theorem my_lemma3 :
    ∀ {x y ε : ℝ}, 0 < ε → ε ≤ 1 → |x| < ε → |y| < ε → |x * y| < ε := by
  intro x y ε epos ele1 xlt ylt -- although still have to declare the variables
  sorry
...
example (mf : Monotone f) (mg : Monotone g) : Monotone fun x ↦ f (g x) := by
  intro a b aleb
  exact RCLike.ofReal_le_ofReal.mp (mf (mg aleb))
